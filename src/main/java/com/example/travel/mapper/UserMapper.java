...
//    List<User> findByContent(String query) ;
    ...
    //查询所有游客
    @Select(" SELECT  uu.id ,uu.name,uu.age,uu.gender,uu.phone,rr.id roleId,rr.role roleName  " +
            " FROM user uu JOIN  user_role ur ON uu.id=ur.uid " +
            " JOIN role rr ON rr.id=ur.rid  " +
            "  WHERE  " +
            "  name  NOT IN ( SELECT  u.name  FROM   user  u  JOIN guide g ON u.name = g.name  )  " +
            "  ORDER BY uu.id ASC")
    List<UserRole> findAll() ;

    @Select("SELECT uu.id ,uu.name,uu.age,uu.gender,uu.phone,rr.id roleName,rr.role roleName  " +
            " FROM user uu JOIN  user_role ur ON uu.id=ur.uid  " +
            " JOIN role rr ON rr.id=ur.rid   " +
            "  WHERE name LIKE  CONCAT('%',#{query},'%')  " +
            " AND name  NOT IN ( SELECT  u.name  FROM   user  u  JOIN guide g ON u.name = g.name )  " +
            " ORDER BY uu.id ASC ")
    List<UserRole> findByContent(String query) ;
